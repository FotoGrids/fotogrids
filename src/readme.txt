...
* **The Templates screen looked empty while it loaded.** The template catalogue is fetched when the page opens, and until it arrived the screen showed a single line of text beside a spinner that never appeared - the markup asked for the WordPress spinner with a class WordPress does not style. The page chrome - the tabs, the filters and the sidebar - is now rendered by WordPress itself, so it is on screen with the rest of the page rather than after the admin scripts have downloaded and started, and the animated FotoGrids mark holds the grid area until the catalogue arrives. The same invisible spinner is replaced in the Apply Template and template overview dialogs, the gallery Templates panel, and the album and gallery manager panels.
