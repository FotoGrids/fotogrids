...
final class Module {
    ...
    public static function init(): void {
        ...
        add_action( 'wp_enqueue_scripts', [ self::class, 'enqueue_frontend_style' ] );
        ...
        // catches our modules.
        add_filter( Filters_Page_Builders::HAS_CONTENT, [ self::class, 'detect_in_divi' ], 10, 2 );

        // Suppress Divi's global overlay/lightbox over FotoGrids anchors
        // during a Divi-sourced render.
        add_filter( Filters_Render::ANCHOR_ATTRS, [ self::class, 'disable_divi_overlay' ], 10, 2 );

        // Force inline asset emission inside the Visual Builder, where
        // wp_head / wp_footer don't fire the way the renderer's default
        // heuristic expects.
        add_filter( Filters_Render::SHOULD_INLINE_ASSETS, [ self::class, 'inline_assets_in_builder' ] );
    }
    ...
    public static function boot_early(): void {
        ...
        add_action(
            ...
            [ self::class, 'register_native_modules' ]
        );
        ...
        add_action( 'et_fb_framework_loaded', [ self::class, 'register_vb_package' ] );
        ...
        add_action( 'divi_visual_builder_assets_before_enqueue_scripts', [ self::class, 'register_vb_package' ] );
        ...
        add_action( 'wp_enqueue_scripts', [ self::class, 'enqueue_vb_bundle_directly' ], 20 );
    }
    ...
    public static function enqueue_vb_bundle_directly(): void {
        ...
        foreach ( [ 'divi-module-library', 'divi-hooks' ] as $dep ) {
    ...
    public static function disable_divi_overlay( array $attrs, Render_Context $render ): array {
        if ( $render->meta->source !== Request_Source::DIVI ) {
            return $attrs;
        }
    ...
    public static function inline_assets_in_builder( bool $should_inline ): bool {
        ...
        if ( isset( $_GET['et_pb_preview'] ) && (string) $_GET['et_pb_preview'] === 'true' ) {
            return true;
        }
