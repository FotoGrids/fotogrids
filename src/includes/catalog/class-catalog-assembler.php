...
final class Catalog_Assembler {
     ...
     */
    private array $registered_origins = [ 'fotogrids' ];
    ...
    private function apply_file( array $catalog_file ): void {
        $placement = $catalog_file['placement'] ?? [
            'mode' => 'insert_tab',
            'position' => 'at_end',
        ];
    ...
    private function apply_insert_subtab( array $catalog_file, array $placement ): void {
        ...
        if ( $parent_tab_id === '' || ! isset( $this->tree[ $parent_tab_id ] ) ) {
            ...
            $this->apply_insert_tab( $catalog_file, [ 'mode' => 'insert_tab', 'position' => 'at_end' ] );
            return;
        }
    ...
    private function apply_insert_section( array $catalog_file, array $placement ): void {
        ...
        if ( $parent_subtab_id !== null ) {
            $subtabs = is_array( $parent_tab['subTabs'] ?? null ) ? $parent_tab['subTabs'] : [];
        ...
        if ( $parent_subtab_id !== null ) {
            ...
            $existing_settings = is_array( $subtab['settings'] ?? null ) ? $subtab['settings'] : [];
            ...
            $existing_settings = is_array( $parent_tab['settings'] ?? null ) ? $parent_tab['settings'] : [];
    ...
    private function apply_extend_options( array $catalog_file, array $placement ): void {
        ...
        $extra_options = $placement['extend_options'] ?? [];
        ...
        array_walk_recursive_subtree(
            ...
            function ( array &$node ) use ( $target_setting_key, $extra_options, &$target_found ): void {
                ...
                $existing_options = is_array( $node['options'] ?? null ) ? $node['options'] : [];
    ...
    private function apply_replace( array $catalog_file, array $placement ): void {
        ...
        foreach ( $this->tree as $tab_id => $tab_node ) {
            if ( isset( $tab_node['subTabs'][ $target_id ] ) ) {
                $this->tree[ $tab_id ]['subTabs'][ $target_id ] = [
                    'id' => $target_id,
        ...
        if ( is_string( $anchor_after ) && $anchor_after !== '' ) {
            foreach ( $existing_settings as $sibling_index => $sibling_setting ) {
                if ( ( $sibling_setting['key'] ?? null ) === $anchor_after ) {
                    array_splice( $existing_settings, $sibling_index + 1, 0, [ $new_setting ] );
        ...
        if ( is_string( $anchor_before ) && $anchor_before !== '' ) {
            foreach ( $existing_settings as $sibling_index => $sibling_setting ) {
                if ( ( $sibling_setting['key'] ?? null ) === $anchor_before ) {
                    array_splice( $existing_settings, $sibling_index, 0, [ $new_setting ] );
                    return $existing_settings;
                }
            }
            $this->warn( sprintf(
    ...
    private function splice_associative( array $existing, int $index, string $new_key, array $new_node ): array {
        ...
        return $before + [ $new_key => $new_node ] + $after;
    }
...
if ( ! function_exists( __NAMESPACE__ . '\\array_walk_recursive_subtree' ) ) {
    /**
     ...
     * @return  void
     */
    function array_walk_recursive_subtree( array &$tree, callable $visit ): void {
        foreach ( $tree as &$child ) {
            ...
            array_walk_recursive_subtree( $child, $visit );
        }
        unset( $child );
    }
}
