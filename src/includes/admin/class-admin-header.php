...
class Admin_Header {
	...
	private function render_header() {
		...
		<div id="fotogrids-header" class="fotogrids-header">
			...
			<div class="fotogrids-links">
				...
				<a href="#" role="button" aria-haspopup="dialog" class="fotogrids-link fotogrids-link-whats-new fotogrids-splash-modal-open">
					<svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	private function get_logo_svg() {
		return \FotoGrids\Svg::fotogrids_logo();
	}

	public function dismiss_notice() {
		$nonce = isset( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';
		if ( ! wp_verify_nonce( $nonce, 'fotogrids_dismiss_notice' ) ) {
	...
	/**
	 * Enqueue the page-structure CSS/JS on FotoGrids gallery/album screens.
	 *
	 * Emitted as inline additions on a dedicated head handle so the fade-in
	 * style applies before first paint and the restructure script runs with
	 * the same head timing as before, without hardcoding raw tags.
	 *
	 * @return void
	 */
	private function enqueue_page_structure() {
		$screen = get_current_screen();
		if ( ! $screen || ! in_array( $screen->post_type, array( 'fotogrids_gallery', 'fotogrids_album' ), true ) ) {
			return;
		}

		wp_register_style( 'fotogrids-admin-structure', false, array(), FOTOGRIDS_VERSION );
		wp_enqueue_style( 'fotogrids-admin-structure' );
		wp_add_inline_style( 'fotogrids-admin-structure', $this->page_structure_css() );

		wp_register_script( 'fotogrids-admin-structure', false, array(), FOTOGRIDS_VERSION, false );
		wp_enqueue_script( 'fotogrids-admin-structure' );
		wp_add_inline_script( 'fotogrids-admin-structure', $this->page_structure_js() );
	}

	/**
	 * CSS that hides the default wp-admin `.wrap` until JS restructures it.
	 *
	 * @return string
	 */
	private function page_structure_css() {
		return <<<'CSS'
	...
	/**
	 * JavaScript that restructures the FotoGrids admin page chrome.
	 *
	 * @return string
	 */
	private function page_structure_js() {
		return <<<'JS'
		(function() {
			'use strict';
