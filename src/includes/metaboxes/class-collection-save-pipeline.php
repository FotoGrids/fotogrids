...
final class Collection_Save_Pipeline {
	...
	public static function init(): void {
		...
		add_action( 'wp_ajax_fotogrids_save_collection', array( __CLASS__, 'on_ajax_save_collection' ) );
	}
	...
	public static function on_ajax_save_collection(): void {
		...
		$collection_type = 'fotogrids_album' === $post->post_type
			? __( 'Album', 'fotogrids' )
	...
	private static function persist_settings_with_gate( int $post_id, array $request_data ): array {
		...
		$skipped      = array();
		if ( null !== $settings_cap && ! Permission_Check::can( $settings_cap, $post_id ) && ! empty( $incoming ) ) {
		...
		if ( null !== $protect_value && ! $protect_value ) {
