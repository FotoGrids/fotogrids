...
class Freemius_License_Provider implements License_Provider {
	...
	private function resolve_freemius_slugs_for_tier( string $tier ): array {
		$tier_to_freemius_plans = apply_filters(
			...
			array(
				'pro_starter' => array( 'pro_starter_01' ),
				'pro_plus'    => array( 'pro_plus_01' ),
				'agency'      => array( 'pro_agency_01' ),
			)
		);

		$slugs = $tier_to_freemius_plans[ $tier ] ?? array( $tier );
	...
	public function get_plan(): ?string {
		...
		if ( null !== $license && ! empty( $license->parent_plan_title ) ) {
	...
	public function get_license_key(): ?string {
		$license = $this->resolve_license();
		if ( null !== $license && ! empty( $license->secret_key ) ) {
	...
	public function get_expiry(): ?int {
		$license = $this->resolve_license();
		if ( null === $license || empty( $license->expiration ) ) {
			return null;
		}
