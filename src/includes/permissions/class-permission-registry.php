...
final class Permission_Registry {
	...
	public static function get_all(): array {
		...
		usort(
			$defs,
			static function ( Permission_Definition $a, Permission_Definition $b ): int {
	...
	public static function get_for_panel( string $panel ): array {
		return array_values(
			array_filter(
				self::get_all(),
				static function ( Permission_Definition $def ) use ( $panel ): bool {
					if ( 'simple' === $panel ) {
						return 'simple' === $def->panel || 'both' === $def->panel;
					}
					if ( 'advanced' === $panel ) {
						return 'advanced' === $def->panel || 'both' === $def->panel;
					}
					return false;
				}
			)
	...
	public static function classify_key( string $key, string $post_type = '' ): string {
		...
		return in_array( $filtered, array( 'content', 'settings' ), true )
			? $filtered
			: $classification;
	}
