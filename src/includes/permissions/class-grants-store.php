...
final class Grants_Store {
	 ...
	 * @var string[]
	 */
	public const GRANTEE_TYPES = array( 'role', 'user', 'token' );
	 ...
	 * @var string[]
	 */
	public const SCOPE_TYPES = array( 'global', 'gallery', 'album' );
	 ...
	 * @var string[]
	 */
	public const STATES = array( 'granted', 'denied' );
	 ...
	 */
	public static function all( array $filter = array() ): array {
		global $wpdb;
		$table = self::table();
		...
		foreach ( array( 'grantee_type', 'grantee_id', 'scope_type', 'capability', 'state' ) as $col ) {
			if ( isset( $filter[ $col ] ) && null !== $filter[ $col ] && '' !== $filter[ $col ] ) {
				$where[]  = "$col = %s";
				$params[] = (string) $filter[ $col ];
			}
		}
		if ( isset( $filter['scope_id'] ) && null !== $filter['scope_id'] ) {
			$where[]  = 'scope_id = %d';
	...
	public static function upsert( array $grant ): bool {
		...
		if ( $existing ) {
			...
			$updated = $wpdb->update( self::table(), $data, array( 'id' => (int) $existing ) );
