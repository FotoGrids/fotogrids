...
final class Permissions_Data {
     ...
     */
    public static function get_registry( $request ): \WP_REST_Response {
    ...
    public static function set_option( $request ) {
        ...
        if ( $key !== 'unauthorised_visibility' ) {
            return new \WP_Error(
    ...
    public static function set_simple( $request ) {
        ...
        foreach ( $def->underlying_caps as $cap ) {
            ...
            if ( in_array( $cap, [ 'manage_fotogrids', 'manage_fotogrids_permissions' ], true ) ) {
                $allowed_roles = array_unique( array_merge( $allowed_roles, [ 'administrator' ] ) );
                $denied_roles  = array_values( array_diff( $denied_roles, [ 'administrator' ] ) );
            }
