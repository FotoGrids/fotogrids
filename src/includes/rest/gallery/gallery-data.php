...
class Gallery_Data {
	...
	public static function set_featured_item( $request ) {
		...
		$clearing   = ( null === $item_param || '' === $item_param || (int) 0 === $item_param );
	...
	public static function get_gallery( $request ) {
		...
		if ( ! $is_preview && 'publish' !== $gallery->post_status ) {
			return new \WP_Error(
	...
	public static function render_gallery( $request ) {
		...
		if ( null !== $meta && null !== $meta->pagination_page_size ) {
			$page_size = (int) $meta->pagination_page_size;
		}
		$total_pages = null !== $meta && null !== $meta->pagination_total_pages
