...
class Items_Data {
	...
	public static function query_items( $request ) {
		...
		$sql            = "SELECT * FROM %i $where_sql ORDER BY position ASC LIMIT %d OFFSET %d";
		$query_params[] = $limit;
		$query_params[] = $offset;

		$results = $wpdb->get_results( $wpdb->prepare( $sql, $query_params ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Custom table; $sql is assembled above with every value passed through prepare().
