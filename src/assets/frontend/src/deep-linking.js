...
(function () {
    ...
    function resolveTarget(link) {
        ...
        for (const galleryEl of galleries) {
            const itemEl = galleryEl.querySelector(`[data-fg-item-id="${link.itemId}"]`);
            if (itemEl) {
                const figure = itemEl.closest('.fg-item') || itemEl;
                const trigger = figure.querySelector('[data-fg-lightbox-trigger]')
                    || (itemEl.matches('[data-fg-lightbox-trigger]') ? itemEl : null);
    ...
    function highlightItem(itemEl) {
        const reveal = () => {
            ...
            setTimeout(() => itemEl.classList.remove('fg-deep-link-highlight'), 2200);
        };

        const img = itemEl.querySelector('img[loading="lazy"], img[data-src]');
        if (img && !img.complete && typeof IntersectionObserver !== 'undefined') {
    ...
    function init() {
        ...
        document.addEventListener('fotogrids:lightbox:navigate', syncUrlToLightbox);
