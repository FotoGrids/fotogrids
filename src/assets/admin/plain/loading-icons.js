...
(async function() {
    ...
    if (!baseUrl) {
        console.warn('FotoGrids: Plugin URL not available, cannot load config/loading-icons-smil.json');
        return;
    }

    try {
        const response = await fetch(`${baseUrl}config/loading-icons-smil.json`);
        if (response.ok) {
            ...
            if (typeof window !== 'undefined') {
                // window.FotoGridsLoadingIcons already points to FotoGridsLoadingIcons;
                // no reassignment needed, but keep it explicit for clarity.
                window.FotoGridsLoadingIcons = FotoGridsLoadingIcons;
            }

            mergeLoadingIcons();
        } else {
            console.warn('FotoGrids: Could not load config/loading-icons-smil.json, using empty object');
        }
    } catch (error) {
        console.warn('FotoGrids: Error loading config/loading-icons-smil.json:', error);
    }
})();
