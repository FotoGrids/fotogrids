...
const FOTOGRIDS_SYSTEM_FONT_OPTIONS = [
    { label: 'Arial', value: 'Arial', fontFamily: 'Arial, sans-serif', source: 'system' },
    { label: 'Helvetica', value: 'Helvetica', fontFamily: 'Helvetica, Arial, sans-serif', source: 'system' },
    { label: 'Times New Roman', value: 'Times New Roman', fontFamily: '"Times New Roman", Times, serif', source: 'system' },
    { label: 'Georgia', value: 'Georgia', fontFamily: 'Georgia, serif', source: 'system' },
    { label: 'Courier New', value: 'Courier New', fontFamily: '"Courier New", Courier, monospace', source: 'system' }
];
    ...
    cache.loadPromise = (async () => {
        ...
        try {
            const response = await fetch(endpoint, {
                ...
                headers: {
                    'X-WP-Nonce': window.fotogridsSettings?.restNonce || window.wpApiSettings?.nonce || ''
                }
            });
...
const buildGoogleFontsStylesheetUrl = (fontFamilies) => {
    const params = fontFamilies.map((family) => {
...
const preloadGoogleFontPreviews = (fontFamilies) => {
    ...
    const sanitizedFamilies = fontFamilies
    ...
    chunkArray(sanitizedFamilies, FOTOGRIDS_GOOGLE_PREVIEW_BATCH_SIZE).forEach((batch) => {
...
const FontFamilyComponent = ({ setting, currentValue, isDisabled, updateSetting, getFieldState, renderIcon, __ }) => {
    ...
    const [googleVisibleCount, setGoogleVisibleCount] = useState(FOTOGRIDS_GOOGLE_RENDER_BATCH_SIZE);
    ...
    useEffect(() => {
        ...
        fetchGoogleFontsFamilies()
            .then((families) => {
                ...
                setGoogleFonts(families.map((family) => ({
                    label: family,
                    value: family,
            ...
            .catch(() => {
                ...
                setGoogleFonts(FOTOGRIDS_GOOGLE_FONTS_FALLBACK.map((family) => ({
                    label: family,
                    value: family,
                    fontFamily: `"${family}", sans-serif`,
                    source: 'google'
    ...
    const filteredSystemOptions = useMemo(() => {
        ...
        return FOTOGRIDS_SYSTEM_FONT_OPTIONS.filter((font) => font.label.toLowerCase().includes(normalizedSearchTerm));
    ...
    const filteredGoogleOptions = useMemo(() => {
        ...
        return googleFonts.filter((font) => font.label.toLowerCase().includes(normalizedSearchTerm));
    ...
    const selectedOption = useMemo(() => {
        ...
        const systemMatch = FOTOGRIDS_SYSTEM_FONT_OPTIONS.find((font) => font.value === resolvedValue);
        if (systemMatch) {
            return systemMatch;
        }

        const googleMatch = googleFonts.find((font) => font.value === resolvedValue);
        if (googleMatch) {
            return googleMatch;
        }
    ...
    const canLoadMoreGoogleFonts = visibleGoogleOptions.length < filteredGoogleOptions.length;
    ...
    const handleDropdownScroll = (event) => {
        ...
        if (distanceToBottom < 40) {
            setGoogleVisibleCount((previousCount) => previousCount + FOTOGRIDS_GOOGLE_RENDER_BATCH_SIZE);
        }
    };

    const googleStatus = googleFontsStatus === 'loading'
        ? __('Loading Google Fonts...', 'fotogrids')
        : googleFontsStatus === 'fallback'
            ? __('Showing popular Google Fonts while full list is unavailable.', 'fotogrids')
            : (normalizedSearchTerm && filteredSystemOptions.length === 0 && filteredGoogleOptions.length === 0
    ...
    return window.FotoGridsRenderSettings.renderSelect({
        ...
        getOptionStyle: (option) => option?.fontFamily ? { fontFamily: option.fontFamily } : undefined,
        rootClassName: 'fotogrids-font-family',
        maxDropdownHeight: 400
    });
};

window.FotoGridsRenderSettings.renderFontFamily = (setting, currentValue, isDisabled, {
    updateSetting,
    getFieldState,
    renderIcon,
    __
