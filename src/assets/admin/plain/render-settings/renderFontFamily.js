...
const FontFamilyComponent = ({
	...
	// Google Fonts opt-out (Plugin Settings > Advanced; on by default). When an
	// admin disables it, hide Google Fonts from the picker entirely so the
	// metadata endpoint is never called and no Google font can be selected.
	const googleFontsEnabled =
		window.fotogridsSettings?.allowGoogleFonts !== false;
	...
	return window.FotoGridsRenderSettings.renderSelect({
		...
		onOpen: () => {
			...
			if (googleFontsEnabled && googleFontsStatus === 'idle') {
