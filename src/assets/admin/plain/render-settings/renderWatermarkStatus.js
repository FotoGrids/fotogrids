...
const WatermarkStatusComponent = ({
    ...
    const request = (path, method) => fetch(`${restUrl}${path}`, {
        method,
    ...
    const fetchStatus = async () => {
        try {
            const response = await request(`admin/watermark/status?gallery_id=${postId}`, 'GET');
    ...
    React.useEffect(() => {
        if (postId) { fetchStatus(); }
    }, [postId]);

    const regenerate = async (ids) => {
        if (isDisabled || running || !Array.isArray(ids) || ids.length === 0) return;
        ...
        if (window.fotogridsToast) {
            window.fotogridsToast.success(__('Watermarks updated for this gallery.', 'fotogrids'));
        }
    };
    ...
    const title = wp.i18n.sprintf(
        ...
        __('%1$d of %2$d items in this gallery aren’t watermarked', 'fotogrids'),
        pending,
        total
    );

    const stateLabel = (state) => state === 'stale'
    ...
    const list = showList && h('ul', {
        key: 'list',
        className: `${baseClass}__list`,
    }, missingItems.map((it) => h('li', {
        key: it.attachment_id,
        className: `${baseClass}__item`,
    }, [
        h('span', { key: 'name', className: `${baseClass}__item-name` }, it.filename || `#${it.attachment_id}`),
        h('span', { key: 'state', className: `${baseClass}__item-state ${baseClass}__item-state--${it.state}` }, stateLabel(it.state)),
    ])));

    return h('div', { className: baseClass }, [
        h('div', { key: 'notice', className: `${baseClass}__notice` }, [
            h('div', { key: 'text', className: `${baseClass}__text` }, [
                h('div', { key: 'title', className: `${baseClass}__title` }, title),
                h('div', { key: 'desc', className: `${baseClass}__desc` },
                    running
                        ? wp.i18n.sprintf(__('Regenerating… %1$d of %2$d', 'fotogrids'), progress.done, progress.total)
                        : __('The site watermark is on for this gallery, but these images still serve clean copies.', 'fotogrids')
                ),
            ]),
            h('div', { key: 'buttons', className: `${baseClass}__buttons` }, [
                h('button', {
                    key: 'btn',
                    type: 'button',
...
window.FotoGridsRenderSettings.renderWatermarkStatus = (setting, currentValue, isDisabled, {
    __,
    postId,
    restUrl,
    restNonce,
