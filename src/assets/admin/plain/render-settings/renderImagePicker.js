...
window.FotoGridsRenderSettings.renderImagePicker = (
	...
	const ImagePicker = () => {
		...
		const openPicker = () => {
			...
			if (!mediaFrameRef.current) {
				mediaFrameRef.current = wp.media({
					title:
						setting.pickerTitle || __('Choose image', 'fotogrids'),
					button: {
						text:
							setting.pickerButton ||
							__('Use this image', 'fotogrids'),
					},
					multiple: false,
					library: { type: 'image' },
				});
				mediaFrameRef.current.on('select', () => {
					const sel = mediaFrameRef.current
						.state()
						.get('selection')
						.first();
					if (sel) {
		...
		return h(
			'div',
			{ className: 'fotogrids-image-picker' },
			[
				previewUrl &&
					h('img', {
						key: 'preview',
					...
					[
						h(
							'button',
							{
								...
								type: 'button',
								className:
									'fg-button fg-button--variant-secondary fg-button--size-sm',
								onClick: openPicker,
								disabled: isDisabled,
							},
							chooseLabel,
						...
						attachmentId > 0 &&
							h(
								'button',
								{
									...
									type: 'button',
									className:
										'fg-button fg-button--ghost fg-button--size-sm',
									onClick: clear,
									disabled: isDisabled,
								},
								__('Remove', 'fotogrids'),
	...
	return h(
		'div',
		{ className: 'fotogrids-image-picker-wrapper' },
		[
			setting.label &&
				h(
					'label',
