...
function buildSelectorHighlightPlugin() {
	...
	if (!cm || !cm.ViewPlugin || !cm.Decoration || !cm.RangeSetBuilder)
		return null;
...
const CodeAreaComponent = ({
	...
	React.useEffect(() => {
		...
		function initializeEditor() {
			...
			const { EditorView, basicSetup, css, javascript } =
	...
	return React.createElement(
		...
		[
			...
			setting.hint_link &&
				(() => {
					// hint_link shape: { label, tab, field }. Uses
					// window.fotogridsAdmin.settingsBaseUrl when available so the URL is
					// correct regardless of WordPress install path; falls back to a
					// relative path that works in standard wp-admin contexts.
					const {
						label: linkLabel,
						tab,
						field,
					...
					const base =
						window.fotogridsAdmin?.settingsBaseUrl ||
					...
					const anchorProps = {
						href,
						className: 'fotogrids-codearea-hint-link__anchor',
					};
