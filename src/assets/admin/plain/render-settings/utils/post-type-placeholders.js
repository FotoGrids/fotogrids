...
const processSettingPlaceholders = (setting, normalizedPostType) => {
    ...
    if (processed.hint) {
        processed.hint = replacePostTypePlaceholders(processed.hint, normalizedPostType);
    }

    if (processed.hint_link && processed.hint_link.label) {
        processed.hint_link = {
            ...processed.hint_link,
            label: replacePostTypePlaceholders(processed.hint_link.label, normalizedPostType)
        };
    }
    ...
    if (processed.subtitle) {
        processed.subtitle = replacePostTypePlaceholders(processed.subtitle, normalizedPostType);
    }

    if (processed.message) {
        processed.message = replacePostTypePlaceholders(processed.message, normalizedPostType);
    }

    if (processed.button_label) {
        processed.button_label = replacePostTypePlaceholders(processed.button_label, normalizedPostType);
    }
...
window.FotoGridsRenderSettings.replacePostTypePlaceholders = replacePostTypePlaceholders;
window.FotoGridsRenderSettings.processSettingPlaceholders = processSettingPlaceholders;
