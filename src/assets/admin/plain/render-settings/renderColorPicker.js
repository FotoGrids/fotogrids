...
function AlphaColorPicker({
    setting, currentValue, isDisabled, updateSetting,
    showSettingBadge, settingBadgeText,
    h, useState, useEffect, useRef, useCallback, createPortal, __,
}) {
    ...
    useEffect(() => {
        ...
        const handleClick = (e) => {
            const clickedInsideTrigger = wrapRef.current && wrapRef.current.contains(e.target);
    ...
    return h('div', {
        ...
        h('div', { className: inputClassName, ref: triggerRef }, [
            ...
            h('input', {
                ...
                onClick: toggleOpen,
                onFocus: () => { setFocused(true); toggleOpen(); },
                onBlur:  () => setFocused(false),
                style: { cursor: isDisabled ? 'not-allowed' : 'pointer' },
            }),
        ]),

        open && popoverPosition && (typeof createPortal === 'function'
            ? createPortal(h('div', {
                className: 'fotogrids-color-popover fotogrids-color-popover--open',
                ref: popoverRef,
                style: {
                    position: 'absolute',
...
function isCompleteColor(str) {
    ...
    const s = str.trim();
    if (/^#[0-9A-Fa-f]{3}$/.test(s) || /^#[0-9A-Fa-f]{6}$/.test(s) || /^#[0-9A-Fa-f]{8}$/.test(s)) return true;
