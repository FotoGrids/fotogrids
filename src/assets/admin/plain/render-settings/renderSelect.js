...
const RenderSelectComponent = ({
    ...
    const listboxIdRef = useRef(`fotogrids-select-${Math.random().toString(36).slice(2, 10)}`);
    ...
    const updateDropdownPosition = useCallback(() => {
        ...
        const placement = availableBelow >= minPanelHeight || availableBelow >= availableAbove
            ? 'bottom'
            : 'top';
        ...
        const resolvedMaxHeight = typeof maxDropdownHeight === 'number' && maxDropdownHeight > 0
    ...
    const renderOption = (option, optionKeyPrefix) => {
        ...
        const customClassName = typeof getOptionClassName === 'function'
        ...
        return h('button', {
            ...
            style: typeof getOptionStyle === 'function' ? getOptionStyle(option, isSelected) : undefined,
            role: 'option',
        ...
        }, typeof renderOptionLabel === 'function' ? renderOptionLabel(option, isSelected) : option.label);
    };
    ...
    const dropdownElement = isOpen && dropdownPosition
        ? h('div', {
            ...
            searchEnabled && h('div', {
                className: 'fotogrids-render-select__search',
                key: 'search'
            }, h('input', {
                ref: searchInputRef,
                type: 'text',
                className: 'fotogrids-input fotogrids-render-select__search-input',
                placeholder: searchPlaceholder || __('Search...', 'fotogrids'),
                value: searchTerm,
                onChange: (event) => {
            ...
            h('div', {
                className: ['fotogrids-render-select__options', optionsClassName].filter(Boolean).join(' '),
                id: listboxId,
                role: 'listbox',
                onScroll: onDropdownScroll
            }, [
                ...topOptions.map((option) => renderOption(option, 'top')),
                ...groups.map((group, groupIndex) => h('div', {
                    className: 'fotogrids-render-select__group',
                    key: group.id || `group-${groupIndex}`
                }, [
                    group.label && h('div', {
                        className: 'fotogrids-render-select__group-label'
                    }, group.label),
                    ...(Array.isArray(group.options) ? group.options.map((option) => renderOption(option, group.id || `group-${groupIndex}`)) : []),
                    group.status && h('div', {
                        className: 'fotogrids-render-select__status'
                    }, group.status)
                ]))
            ])
        ])
    ...
    const portalElement = dropdownElement && typeof createPortal === 'function'
        ? createPortal(dropdownElement, document.body)
        : dropdownElement;

    return h('div', {
        className: ['fotogrids-render-select', rootClassName, isOpen ? 'fotogrids-render-select--is-open' : ''].filter(Boolean).join(' ')
    }, [
        ...
        h('button', {
            type: 'button',
            ref: triggerRef,
            className: ['fotogrids-render-select__trigger', triggerClassName].filter(Boolean).join(' '),
            disabled: isDisabled,
        ...
        h('span', {
            ...
            style: typeof getOptionStyle === 'function' ? getOptionStyle(selectedOption, true) : undefined
