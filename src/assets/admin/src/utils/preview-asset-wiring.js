 ...
 *
 *   1b. applyPreviewInlineCss(instanceId, inlineCss)
 *      The settings-driven custom properties Inline_Asset_Emitter would print
 *      on a real page. Re-applied on every response, not deduped.
 *
...
/**
 * Applies the per-render inline CSS - the settings-driven custom properties
 * (caption fonts and colours, gaps, column counts, borders, the responsive
 * media queries) scoped to this gallery instance.
 *
 * On a real page Inline_Asset_Emitter prints these; it self-gates out of REST
 * so the preview endpoint returns them as `inlineCss` instead. Unlike the
 * handle-based assets this must NOT be deduped: every settings edit produces a
 * new response with new values for the same instance. One <style> per instance
 * id, rewritten in place and moved to the end of head so it still wins the
 * cascade after later handles append their links.
 *
 * @param {string}   instanceId      Gallery instance id (response.instance_id).
 ...
 *      REST endpoints.
 *   2. Ensure CSS handles (idempotent) and re-apply the inline CSS vars.
