...
const TOAST_DEFAULTS = {
    success: { key: 'copiedMessage', message: 'Shortcode copied to clipboard', duration: 1500 },
    error:   { key: 'copyErrorMessage', message: 'Failed to copy Shortcode', duration: 3000 },
};
...
export function showToast(type, message) {
    ...
    const localized = window.fotogridsShortcodeColumn && window.fotogridsShortcodeColumn[def.key];
        ...
        button.addEventListener('click', async (e) => {
            ...
            if (!text) {
                if (typeof console !== 'undefined' && console.warn) {
                    console.warn('FotoGrids: No shortcode found on copy button');
                }
                return;
            }
            try {
