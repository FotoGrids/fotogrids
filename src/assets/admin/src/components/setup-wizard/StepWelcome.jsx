...
const TELEMETRY_PERMISSIONS = [
	...
	},
	{
		...
		body: __(
			'Homepage URL & title, WordPress and PHP versions, site language.',
			'fotogrids'
		),
	},
	{
		...
		body: __(
			'Current FotoGrids & SDK versions, and whether they are active or being uninstalled.',
			'fotogrids'
		),
	},
	{
		...
		body: __(
			'Names, slugs, versions, and whether each one is active. Helps us catch conflicts early.',
			'fotogrids'
		),
	},
];
 ...
 * Step 1 - Welcome.
 *
 * Front door for the wizard. Sets the tone, captures the usage-data
...
const StepWelcome = ({ onStart, onSkip }) => {
	const initialOptIn = !!(
		window.fotogridsAdmin && window.fotogridsAdmin.shareStatistics
	);
	const [optedIn, setOptedIn] = useState(initialOptIn);
	const [marketingOptedIn, setMarketingOptedIn] = useState(
		!!(window.fotogridsAdmin && window.fotogridsAdmin.marketingAllowed)
	);
	...
	const handleTelemetryToggle = (next) => {
		...
		// Marketing consent can't outlive usage-data sharing. Turning the
		// first toggle off revokes it server-side, so mirror that here.
		if (!next && marketingOptedIn) {
			setMarketingOptedIn(false);
			if (window.fotogridsAdmin) {
				window.fotogridsAdmin.marketingAllowed = false;
			}
		}
	};

	const handleMarketingToggle = (next) => {
		setMarketingOptedIn(next);
		persistSetting('fotogrids_marketing_allowed', next);
	};
			...
			<div className="fotogrids-setup__step--welcome__content">
				...
				<div className="fotogrids-setup__step--welcome__content-inner">
					...
					<p className="fotogrids-setup__step-subhead">
						{__(
							'Create your first gallery in 5 easy steps!',
							'fotogrids'
						)}
					</p>
				</div>
				...
				<div className="fotogrids-setup__consent">
					<Toggle
						...
						label={__(
							'Help improve FotoGrids by sharing usage data',
							'fotogrids'
						)}
						description={
							<>
								{__(
									'You can turn this off anytime in FotoGrids Settings → Advanced.',
									'fotogrids'
								)}{' '}
								<button
									type="button"
					...
					{optedIn && (
						<Toggle
							id="fotogrids-setup-marketing"
							checked={marketingOptedIn}
							onChange={handleMarketingToggle}
							label={__(
								'Get occasional FotoGrids news and offers by email',
								'fotogrids'
							)}
							description={__(
								'Optional and separate from usage tracking. You’ll only receive emails if you turn this on, and you can unsubscribe at any time.',
								'fotogrids'
							)}
						/>
					)}
			...
			>
				...
				<Modal.Body>
					<p className="fotogrids-setup-telemetry-modal__intro">
						{__(
							'Sharing this data is optional. It helps us constantly improve by catching bugs early, prioritising what to build next, and keeping FotoGrids compatible with your setup. None of it is sold, and we never collect image content, image URLs, or visitor data.',
							'fotogrids'
						)}
					</p>
					...
					>
						{TELEMETRY_PERMISSIONS.map((p) => (
							<li
								key={p.id}
								className="fotogrids-setup-telemetry-modal__item"
							>
								<span
									className="fotogrids-setup-telemetry-modal__item-icon"
									aria-hidden="true"
								>
									<Icon name={p.icon} />
								</span>
								<span className="fotogrids-setup-telemetry-modal__item-body">
									<span className="fotogrids-setup-telemetry-modal__item-title">
										{p.title}
									</span>
									<span className="fotogrids-setup-telemetry-modal__item-text">
										{p.body}
									</span>
								</span>
							</li>
					...
					<p className="fotogrids-setup-telemetry-modal__footnote">
						<span>
							{__(
								'You can turn this off any time from FotoGrids → Settings → Advanced.',
								'fotogrids'
							)}
						</span>
						<a
