...
export async function persistSetting( setting, value ) {
    ...
    if ( ! ajaxUrl || ! nonce ) {
        ...
        console.warn( 'FotoGrids: missing ajax url / nonce; persistSetting noop' );
        return { ok: false };
    }
    ...
    body.append( 'value', typeof value === 'boolean'
        ? ( value ? '1' : '0' )
        : String( value )
    );
    ...
    try {
        ...
        if ( ! res.ok ) {
            // eslint-disable-next-line no-console
            console.warn( `FotoGrids: persistSetting ${ setting } HTTP ${ res.status }` );
            return { ok: false };
        }
        ...
        // full page reload.
        const saved = json.data && Object.prototype.hasOwnProperty.call( json.data, 'value' )
            ? json.data.value
            : value;
