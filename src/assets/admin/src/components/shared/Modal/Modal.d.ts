...
export interface ModalSubcomponents {
    Header:        ComponentType<{ divider?: boolean; closeButton?: boolean; size?: 'sm' | 'md'; className?: string; children?: ReactNode }>;
    HeaderTitle:   ComponentType<{ level?: 1 | 2 | 3; as?: keyof JSX.IntrinsicElements; className?: string; children?: ReactNode }>;
    ...
    HeaderClose:   ComponentType<{ icon?: string; ariaLabel?: string; onClick?: () => void; className?: string }>;
    Body:          ComponentType<{ padding?: boolean; scroll?: boolean; className?: string; children?: ReactNode }>;
    ...
    Footer:        ComponentType<{ align?: 'left' | 'right' | 'between'; divider?: boolean; className?: string; children?: ReactNode }>;
    Nav:           ComponentType<{ direction: 'prev' | 'next'; onClick?: () => void; disabled?: boolean; ariaLabel?: string; className?: string }>;
}
...
// ─── Public surface on window.FotoGridsAdmin.modal ───────────────────────────
...
export interface FotoGridsAdminModalApi {
    ...
    on:  <K extends ModalEventName>(event: K, handler: (e: CustomEvent) => void) => () => void;
    off: <K extends ModalEventName>(event: K, handler: (e: CustomEvent) => void) => void;
...
declare module '@fotogrids/admin/fg-modal' {
    ...
    export function emit(event: ModalEventName, detail?: Record<string, unknown>): void;
    export function on(event: ModalEventName, handler: (e: CustomEvent) => void): () => void;
    export function off(event: ModalEventName, handler: (e: CustomEvent) => void): void;
}

// ─── Window globals augmentation ─────────────────────────────────────────────
