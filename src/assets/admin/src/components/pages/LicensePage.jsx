...
const LicensePage = () => {
	...
	const fetchLicenseStatus = async () => {
		...
		try {
			...
			setError(
				err.message ||
					__('Failed to fetch license status.', 'fotogrids')
			);
		} finally {
			setLoading(false);
		}
	...
	const handleActivate = async (e) => {
		...
		try {
			...
			if (response.success) {
				setSuccess(
					response.message ||
						__('License activated successfully!', 'fotogrids')
				);
				setLicenseKey('');
			...
			setError(
				err.message || __('Failed to activate license.', 'fotogrids')
			);
		} finally {
			setActivating(false);
		}
	...
	const handleDeactivate = async () => {
		...
		try {
			...
			if (response.success) {
				setSuccess(
					response.message ||
						__('License deactivated successfully.', 'fotogrids')
				);
				await fetchLicenseStatus();
			} else {
				setError(
					response.message ||
						__('License deactivation failed.', 'fotogrids')
				);
			}
		} catch (err) {
			setError(
				err.message || __('Failed to deactivate license.', 'fotogrids')
			);
		} finally {
			setActivating(false);
		}
	...
	const isFree = !isPro;
	const planName =
		licenseStatus && licenseStatus.plan ? licenseStatus.plan : null;
				...
				<div className="fotogrids-license-notice fotogrids-license-error">
					...
					>
						<span className="screen-reader-text">
							{__('Dismiss this notice.', 'fotogrids')}
						</span>
					</button>
				</div>
			)}
			...
			<div className="fotogrids-admin-blocks-grid">
				<div className="fotogrids-admin-block-card fg-abc-license">
					...
					<div className="fotogrids-license-card-content">
						<div
							className={`fotogrids-license-card-title${isPro ? ' fg-license-card-title-pro' : ''}`}
						>
							{licenseTitle}
						</div>
						<div className="fotogrids-license-card-subtitle">
							{licenseSubtitle}
						</div>
							...
							<div className="fotogrids-license-card-details">
								{licenseStatus && licenseStatus.license_key && (
									<div className="fotogrids-license-detail-row">
										<span className="fotogrids-license-detail-label">
											{__('License key', 'fotogrids')}
										</span>
								...
								{licenseStatus &&
									licenseStatus.activations !== null &&
									licenseStatus.quota !== null && (
										<div className="fotogrids-license-detail-row">
											<span className="fotogrids-license-detail-label">
												{__('Activations', 'fotogrids')}
											</span>
											<span className="fotogrids-license-detail-value">
												{licenseStatus.activations} /{' '}
												{licenseStatus.quota}
											</span>
										</div>
									)}
								...
								{licenseStatus &&
									licenseStatus.is_cancelled && (
										<div className="fotogrids-license-detail-row fotogrids-license-detail-row--warning">
											...
											<span className="fotogrids-license-detail-value">
												{__(
													'Cancelled - license remains active until expiry.',
													'fotogrids'
												)}
											</span>
										</div>
								...
								<div className="fotogrids-license-detail-row fotogrids-license-actions">
									...
									>
										{activating
											? __('Deactivating…', 'fotogrids')
											: __('Deactivate', 'fotogrids')}
									</button>
								</div>
								...
								{licenseStatus && licenseStatus.is_opted_in && (
									<div className="fotogrids-license-optin-status">
										<Icon
											name="check"
											className="fotogrids-license-optin-icon"
										/>
										<span className="fotogrids-license-optin-text">
											{__(
												'Opted in to update notifications',
												'fotogrids'
											)}
										</span>
										<a
											...
											aria-label={__(
												'Learn more about update notifications and how to opt out',
												'fotogrids'
											)}
											title={__(
												'Learn more about update notifications and how to opt out',
				...
				{licenseStatus && licenseStatus.needs_pro_install && (
					<div className="fotogrids-admin-block-card fg-abc-download-pro">
						<div className="fotogrids-admin-block-card-header">
							...
							/>
							<h3>
								{__('Download the Pro plugin', 'fotogrids')}
							</h3>
						</div>
						<div className="fotogrids-admin-block-card-content">
							<p>
								{__(
									'Your Pro license is active on this site. Download the Pro plugin and install it alongside FotoGrids to unlock Pro features.',
									'fotogrids'
								)}
							</p>
				...
				{isFree && (
					<div className="fotogrids-admin-block-card fg-abc-upgrade">
						<div className="fotogrids-admin-block-card-content">
							...
							<p>
								{__(
									'Plus many more powerful features designed to save time, optimize performance, and help you grow like never before.',
									'fotogrids'
								)}
							</p>
							<div className="fg-abc-buttons">
