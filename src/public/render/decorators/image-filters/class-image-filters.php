...
final class Image_Filters implements Decorator {
    ...
    public function wrapper_data_attrs( Render_Context $render_context ): array {
        ...
        $thumb_enabled = $this->setting_to_bool( $render_context->settings['thumbnail_filter_enabled'] ?? false );
        if ( $thumb_enabled ) {
            $types = $this->decode_filter_types( $render_context->settings['thumbnail_filter_type'] ?? [] );
            if ( ! empty( $types ) ) {
        ...
        $full_enabled = $this->setting_to_bool( $render_context->settings['full_image_filter_enabled'] ?? false );
        if ( $full_enabled ) {
            $types = $this->decode_filter_types( $render_context->settings['full_image_filter_type'] ?? [] );
            if ( ! empty( $types ) ) {
    ...
    public function style_vars( Render_Context $render_context ): array {
        ...
        if ( $thumb_enabled ) {
            $thumb_types = $this->decode_filter_types( $settings['thumbnail_filter_type'] ?? [] );
        ...
        if ( $full_enabled ) {
            $full_types = $this->decode_filter_types( $settings['full_image_filter_type'] ?? [] );
            if ( ! empty( $full_types ) ) {
    ...
    private function build_responsive_filter_var(
        ...
        foreach ( $breakpoints as $bp ) {
            ...
            foreach ( $filter_types as $type ) {
                ...
                $resolved = $this->resolve_responsive_value( $settings[ $key ] ?? [], $bp, $meta['unit'] );
                if ( $resolved !== '' ) {
                    $parts[] = $type . '(' . $resolved . ')';
                }
            }
