 ...
 * Tags filter source.
 *
 * Provides one Filter_Option per tag that is assigned to at least one item in
 * the current gallery. Options are ordered alphabetically by tag name.
 *
 * Active when:
 *   - filtering_enabled is truthy
 *   - filter_by (token_select) contains 'tags'
 *
 * The corresponding decorator (Tags_Filter_Decorator) stamps data-fg-tags on
 * each gallery item. This source declares the same item_data_attr_key so the
 * JS filter engine knows which attribute to read.
 *
 ...
 * @since   1.0.0
 */
final class Tags_Filter_Source implements Filter_Source {
    ...
    /**
     * Returns tag options with item counts for the current gallery.
     *
     * Queries fotogrids_item_metadata joined to fotogrids_tags for attachment
     * IDs that appear in the render context. Returns options ordered A-Z by
     * tag name, omitting tags with count 0.
     *
     * @since  1.0.0
     */
    ...
    /**
     * No additional assets required for the Tags source itself.
     *
     * @since  1.0.0
     */
