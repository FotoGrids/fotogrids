...
final class Snap_Resolver {
     ...
     */
    private const ASSUMED_WIDTH_DEFAULTS = [
        'desktop' => 1200,
        'tablet'  => 760,
        'mobile'  => 400,
    ];
    ...
    public static function resolve( array $args ): array {
        ...
        $direction        = in_array( $args['direction'] ?? 'auto', [ 'auto', 'prefer_remove', 'prefer_add' ], true )
            ? $args['direction']
            : 'auto';
    ...
    private static function pick_candidate(
        ...
        $clean_candidates = array_values( array_filter(
            $candidates,
        ...
        usort( $pool, static function ( array $a, array $b ) use ( $effective_direction, $target_page_size ) {
            $delta_a = abs( $a['page_size'] - $target_page_size );
            $delta_b = abs( $b['page_size'] - $target_page_size );
            if ( $delta_a !== $delta_b ) {
                return $delta_a <=> $delta_b;
            }
            if ( $effective_direction === 'prefer_remove' ) {
