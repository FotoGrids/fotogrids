...
final class Collection_Header implements Feature {
    ...
    public function supports( Render_Context $render_context ): bool {
        if ( $render_context->meta->collection_kind !== Collection_Kind::GALLERY ) {
            return false;
        }
    ...
    private function resolve( Render_Context $render_context ): ?array {
        ...
        $key = $gallery_id . '|' . ( $via_album_id ?: 0 ) . '|' . ( $render_context->meta->view_page ? 'v' : 'e' );
        if ( $memo_key === $key ) {
            return $memo_val;
        }
        ...
        $is_direct_visit = $render_context->via_album_id === null;
        ...
        $placements_raw = $album_settings['navigation_breadcrumbs_placements'] ?? [ 'view_pages', 'embedded' ];
        if ( is_string( $placements_raw ) && $placements_raw !== '' && $placements_raw[0] === '[' ) {
            $decoded        = json_decode( $placements_raw, true );
            $placements_raw = is_array( $decoded ) ? $decoded : [];
        }
        $placements = is_array( $placements_raw ) ? array_map( 'strval', $placements_raw ) : [];
