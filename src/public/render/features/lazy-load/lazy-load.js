...
( function () {
    ...
    function ensureObservers( galleryEl ) {
        ...
        const state = { native: null, dataSrc: null };
    ...
    function bindImages( galleryEl, scope ) {
        const state = ensureObservers( galleryEl );
        const scopes = Array.isArray( scope ) ? scope : [ scope ];

        scopes.forEach( ( root ) => {
            if ( ! root || ! root.querySelectorAll ) return;

            const nativeLazyImages = root.querySelectorAll( 'img[loading="lazy"]:not(.fotogrids-lazy-loaded)' );
            const dataSrcImages    = root.querySelectorAll( 'img[data-src]' );
