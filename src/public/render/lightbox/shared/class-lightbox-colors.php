...
final class Lightbox_Colors {
     ...
     * @var array<string, string>
     */
    private const CUSTOM_SETTING_KEYS = [
    ...
    public static function theme( array $settings ): string {
        ...
        return in_array( $theme, [ 'dark', 'light', 'custom' ], true ) ? $theme : 'dark';
    }
    ...
    public static function palette( array $settings ): array {
        ...
        $defaults = $theme === 'light' ? self::LIGHT_DEFAULTS : self::DARK_DEFAULTS;
    ...
    public static function attrs( array $settings ): array {
        ...
        foreach ( self::ATTR_KEYS as $key => $attr_name ) {
            ...
            if ( $key === 'info_block_bg' || $key === 'info_block_divider' ) {
                continue;
            }
     ...
     * @return string
     */
    public static function safe_color( mixed $value, string $default ): string {
        if ( ! is_string( $value ) ) {
            return $default;
        }
        $v = trim( $value );
