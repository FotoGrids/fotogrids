...
final class Hooks {
    ...
    public static function fire_action( string $hook_name, Render_Context $render, mixed ...$extra ): void {
        ...
        $hook_arguments = array_merge( $extra, [ $render ] );
    ...
    private static function collection_scope( Render_Context $render ): array {
        $is_album_context = $render->meta->source === Request_Source::ALBUM_AJAX && $render->meta->album_id !== null;

        if ( $is_album_context ) {
            return [ 'album', $render->meta->album_id ];
        }

        return [ 'gallery', $render->meta->gallery_id ];
    }
}
