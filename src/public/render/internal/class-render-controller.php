...
final class Render_Controller {
	...
	public function render( Render_Context $render ): Render_Result {
		...
		try {
			...
			$render           = $render->with( array( 'items' => $collection_items ) );

			$layout_module = $this->select_layout( $render );
			if ( null === $layout_module ) {
			...
			// Paginating layouts get the track and pagination chrome in one
			// fg-layout-body container; layouts that opt out of paginates() skip it.
	...
	private function build_wrapper(
		...
		$base_classes    = Collection_Kind::ALBUM === $render->meta->collection_kind
			? array( 'fotogrids-collection', 'fotogrids-album' )
			: array( 'fotogrids-collection', 'fotogrids-gallery' );
		...
		// read it directly.
		$ordered_attrs = array( 'data-fg-gallery-id' => (string) $render->meta->gallery_id );
		if ( Collection_Kind::ALBUM === $render->meta->collection_kind && null !== $render->meta->album_id ) {
			$ordered_attrs['data-fg-album-id'] = (string) $render->meta->album_id;
		}
		foreach ( $wrapper_data_attrs as $attr_name => $attr_value ) {
			if ( 'data-fg-gallery-id' !== $attr_name && 'data-fg-album-id' !== $attr_name ) {
