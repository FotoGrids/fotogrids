...
final class Layout_Wrapper_Composer {
    ...
    private const ADAPTERS = [
        'enforces_item_box' => [ self::class, 'item_box_adapter' ],
        'uses_columns'      => [ self::class, 'columns_adapter' ],
        'uses_item_spacing' => [ self::class, 'item_spacing_adapter' ],
        'lightbox_extends'  => [ self::class, 'lightbox_extended_adapter' ],
    ];
    ...
    public static function compose( Layout $layout, Render_Context $render_context ): array {
        ...
        return [
            'style_vars'         => is_array( $style_vars ) ? $style_vars : [],
            'wrapper_data_attrs' => is_array( $wrapper_data_attrs ) ? $wrapper_data_attrs : [],
        ];
    }
    ...
    private static function columns_adapter( Layout $layout, Render_Context $render_context ): array {
        ...
        $default_columns    = is_array( $defaults['columns'] ?? null ) ? $defaults['columns'] : [];
        $default_auto_range = is_array( $defaults['columns_auto_range'] ?? null ) ? $defaults['columns_auto_range'] : [];
        ...
        if ( $mode === Columns_Mode::FIXED->value ) {
    ...
    private static function item_spacing_adapter( Layout $layout, Render_Context $render_context ): array {
        ...
        $default_spacing    = is_array( $defaults['item_spacing'] ?? null ) ? $defaults['item_spacing'] : [];
    ...
    private static function lightbox_extended_adapter( Layout $layout, Render_Context $render_context ): array {
        ...
        if ( ! $is_lightbox_click || $scope !== 'gallery' ) {
            return [ 'wrapper_data_attrs' => [] ];
        }
