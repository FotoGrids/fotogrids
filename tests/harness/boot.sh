...
# "~/Local Sites".
write_wp_shim() {
  local php="$1" wp_path="$2" extra="${3:-}"
  mkdir -p "$STATE_DIR"
  cat > "$STATE_DIR/wp" <<EOF
#!/bin/sh
# Generated by tests/harness/boot.sh - do not edit, do not commit.
cd '$(sq "$wp_path")' || exit 1
exec '$(sq "$php")' -d display_errors=stderr '$(sq "$STATE_DIR/wp-cli.phar")' \
  --path='$(sq "$wp_path")' $extra "\$@"
EOF
  chmod +x "$STATE_DIR/wp"
  WP_SHIM="$STATE_DIR/wp"
}
...
boot_local() {
  ...
  have php || die "php not on PATH. Open LocalWP > right-click the site > 'Open site shell', and run this from there."

  FG_LOCAL_PHP="$(local_php || true)"
  if [ -n "$FG_LOCAL_PHP" ]; then
    step "Using LocalWP's php ($("$FG_LOCAL_PHP" -d display_errors=stderr \
      -r 'echo PHP_VERSION;' 2>/dev/null))"
  else
    FG_LOCAL_PHP="$(command -v php)"
    say "  note: LocalWP's php is not on PATH. Falling back to $FG_LOCAL_PHP,"
    say "        which is probably not the build serving the site."
  fi

  fetch_wp_cli
  write_wp_shim "$FG_LOCAL_PHP" "$public_dir"
